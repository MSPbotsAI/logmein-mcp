...
from logmein_mcp.__main__ import _build_http_app
from logmein_mcp.config import Settings
from logmein_mcp.server import create_mcp_server, get_client_from_context
...
def test_missing_both_headers_returns_401_with_required_headers_listed():
    app, _ = _make_app()
    with TestClient(app) as client:
        resp = client.post(
            "/mcp",
        ...
        body = resp.json()
        assert "X-LogMeIn-Username" in body["required_headers"]
        assert "X-LogMeIn-Password" in body["required_headers"]


def test_missing_password_header_returns_401():
    app, _ = _make_app()
    with TestClient(app) as client:
        resp = client.post(
            "/mcp",
            ...
            headers={
                ...
                "X-LogMeIn-Username": "user@example.com",
            },
        )
        assert resp.status_code == 401


def test_both_headers_present_reach_request_context(monkeypatch):
    ...
    from logmein_mcp.server import GatewayTokenMiddleware, _gateway_creds_var
    ...
    async def run():
        scope = {
            ...
            "path": "/mcp",
            "headers": [
                (b"x-logmein-username", b"user@example.com"),
                (b"x-logmein-password", b"hunter2"),
            ],
        }
    ...
    asyncio.run(run())
    assert seen["creds"] == ("user@example.com", "hunter2")
