...
def cap_text(text: str, max_chars: int = MAX_CHARS) -> str:
    """Truncate a raw (non-JSON) string return value to the same char
    budget as dump_json_capped, with an explicit truncation notice appended.

    LogMeIn Rescue's API replies with plain pipe-delimited text (not JSON),
    so tool results here are returned as-is rather than run through
    dump_json_capped — but they still need the same §2.5 size ceiling.
    """
